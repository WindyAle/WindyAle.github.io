...
public: true
badges: ["sLLM Fine-tuning", "RAG", "STT/TTS", "React"]
...
- **상담 중 실시간 지원**: VAD → STT → 키워드 추출 → RAG 검색 → 답변 가이드 제안
- **상담 후 자동화**: 상담 내용 요약, 후처리 문서 생성, AI 기반 피드백
- **교육 시뮬레이션**: 실제 상담 데이터 기반 AI 가상 고객과 훈련

## 담당 역할

### 1. 핵심 키워드 추출 시스템
- 한국어 형태소 분석기 **Kiwi**로 STT 결과에서 명사 추출
- 금융 도메인 **유의어 사전** 구축 → 정확한 용어로 보정 → 검색 정확도 향상

### 2. AI 가상 고객 시뮬레이션 시스템
- **sLLM Fine-tuning**: Kanana-nano 모델을 6,533건 실제 상담 데이터로 학습
- 나이대, 성향, 문의 유형, 말투를 조합한 **고객 페르소나** 생성
- **TTS 음성 재생**으로 실전과 동일한 교육 환경 구현

### 3. 고객 성향 분석 모델 Fine-tuning
- 클래스 재구성과 데이터 증강으로 **정확도 37.1% → 83.4% (+47.4%)** 달성
